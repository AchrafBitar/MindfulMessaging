// Message Detector - Extracts text from various messaging platforms
...
class MessageDetector {
  ...
  private isMessageElement(element: Element): boolean {
    const platform = this.platforms[this.currentPlatform as keyof typeof this.platforms];
    ...
    for (const selector of platform.selectors) {
      try {
        const textElement = element.querySelector(selector) || 
                 ...
                 if (textElement) {
           text = textElement.textContent || (textElement as HTMLElement).innerText || '';
           break;
         }
      } catch (error) {
  ...
  private extractSender(element: Element): string {
  ...
  private extractLatestMessages(): void {
    const platform = this.platforms[this.currentPlatform as keyof typeof this.platforms];
    if (!platform) return;
  ...
  private getRecentMessageElements(): Element[] {
    const platform = this.platforms[this.currentPlatform as keyof typeof this.platforms];
    if (!platform) return [];
  ...
  private handleAnalyzeRequest(payload: any): void {
    try {
  ...
  private extractWhatsAppContext(): any {
  ...
  private extractMessengerContext(): any {
    const titleElement = document.querySelector('[data-testid="conversation-title"]') ||
  ...
  private extractGmailContext(): any {
  ...
  private extractSlackContext(): any {
  ...
  private sendToBackground(messageData: MessageData): void {
    try {
      chrome.runtime.sendMessage({
        type: 'ANALYZE_TEXT',
        payload: {
...
(window as any).mindfulMessageDetector = messageDetector;
